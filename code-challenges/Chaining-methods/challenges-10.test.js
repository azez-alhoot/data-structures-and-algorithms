...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    let about = 'Abidalaziz Alissa, Front-End Developer';
    res.status(200).json(about);
  });


  app.get('/favoritefoods', (req, res) => {
    let favFood = ['Mansaf','Kuftah','Mahashi','Mashawi'];
    res.status(200).json(favFood);
  });

  var server = app.listen(3301, function () {
    var port = server.address(404).port;
...
const count = (target, input) => {
  ...
  let numbers = [];
  let counter = 0;
  let allNum = input.map(value => {
    value.map(val => {
      numbers.push(val)
    });
    counter = numbers.reduce((acc, val, idx) => {
      if (val === target) {
        acc += 1;
      }
      return acc;
